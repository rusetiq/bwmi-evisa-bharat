      ...
      <header className="border-b border-white/15 bg-[var(--espresso)] text-white">
        <div className="shell">
          <div className="flex flex-wrap items-center justify-between gap-5 py-5">
            <Link className="focus-ring inline-flex items-center gap-3 rounded-lg" to="/admin" aria-label="Review desk home">
          ...
          <nav aria-label="Review desk navigation" className="flex flex-wrap items-center gap-x-6 gap-y-1 border-t border-white/15 py-3">
            <NavLink
              to="/admin"
              end
...
export function AdminSection({ title, eyebrow, children, className }: { title: string; eyebrow?: string; children: ReactNode; className?: string }) {
  return (
    <section className={cn('border-t border-[var(--hairline)] pt-5', className)} aria-labelledby={`admin-section-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}>
      ...
      <h2 id={`admin-section-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="display mt-2 text-3xl leading-tight">{title}</h2>
