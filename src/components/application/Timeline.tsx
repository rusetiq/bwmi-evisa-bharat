  ...
  return (
    <ol className="grid gap-0 md:grid-cols-5">
      {timelineLabels.map((label, index) => {
        ...
        return <li key={label} className="relative flex min-w-0 gap-3 border-l border-[var(--hairline)] pb-6 pl-5 last:border-l-0 md:block md:border-l-0 md:pb-0 md:pl-0 md:pr-5">
          ...
          <p className={`mt-1 break-words text-sm md:mt-3 ${current ? 'font-medium' : 'text-stone'}`}>{label}</p>
