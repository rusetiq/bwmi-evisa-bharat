...
export default function AdminApplications() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      query.set('limit', '200')
      try {
        const result = await api.adminApplications(`?${query.toString()}`)
  ...
  return (
    <AdminShell>
      <AdminPageHeader
        eyebrow="Review desk · applications"
      ...
      <section className="mt-8 rounded-2xl border border-[var(--hairline)] bg-[var(--paper)] p-5 sm:p-6" aria-labelledby="filters-heading">
        ...
        <form className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.5fr)_repeat(3,minmax(0,1fr))_auto] lg:items-end" onSubmit={applyFilters}>
          ...
          <Button type="submit" leadingIcon={<Search size={15} aria-hidden="true" />}>Apply</Button>
        </form>
      ...
      <section className="mt-10" aria-labelledby="results-heading" aria-live="polite">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--hairline)] pb-4">
          <div><p className="eyebrow text-stone">Application list</p><h2 id="results-heading" className="display mt-2 text-3xl">{loading ? 'Loading applications' : `${applications.length} application${applications.length === 1 ? '' : 's'}`}</h2></div>
          <Button variant="secondary" leadingIcon={<RefreshCw size={15} aria-hidden="true" />} onClick={() => setRefreshKey((value) => value + 1)} disabled={loading}>Refresh</Button>
        </div>
        <div className="mt-5">
