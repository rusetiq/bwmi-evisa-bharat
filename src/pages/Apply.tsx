...
export default function Apply() {
  ...
  return <main className="shell py-12 sm:py-20"><div className="grid gap-12 lg:grid-cols-[minmax(0,760px)_280px] lg:gap-20"><div><Link className="link inline-flex items-center gap-2 text-sm" to="/">Back to e-Visa home</Link><div className="mt-10"><SectionHeading eyebrow="Start an application" title="Begin with the essentials." description="This creates a saved application workspace. You can complete the remaining details at your own pace." /></div>{error && <div className="mb-6"><Notice title="We couldn't create that application." tone="danger">{error}</Notice></div>}<form className="space-y-6" onSubmit={submit} noValidate><div className="grid gap-6 sm:grid-cols-2"><SelectField id="nationality" label="Nationality" value={nationality} onChange={setNationality} options={countryOptions} required error={fieldErrors.nationality} /><SelectField id="passport-type" label="Passport type" value={passportType} onChange={setPassportType} options={[{ value: 'Ordinary', label: 'Ordinary passport' }, { value: 'Diplomatic', label: 'Diplomatic passport' }, { value: 'Official', label: 'Official or service passport' }]} required /></div><div className="grid gap-6 sm:grid-cols-2"><SelectField id="visa-category" label="Visa category" value={visaCategory} onChange={changeCategory} options={visaCategories} required error={fieldErrors.visaCategory} /><SelectField id="visa-subtype" label="Visa subtype" value={visaSubtype} onChange={setVisaSubtype} options={subtypes} required error={fieldErrors.visaSubtype} /></div><div className="grid gap-6 sm:grid-cols-2"><TextField id="arrival" label="Proposed arrival date" type="date" value={proposedArrival} onChange={setProposedArrival} required error={fieldErrors.proposedArrival} /><TextField id="email" label="Email address" type="email" value={email} onChange={setEmail} placeholder="you@example.com" required error={fieldErrors.email} /></div><div className="flex flex-col gap-4 border-t border-[var(--hairline)] pt-6 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-xs leading-5 text-stone">Mock prototype: your details are saved to this demonstration service only.</p><button type="submit" className="btn btn-primary w-full gap-2 sm:w-auto" disabled={busy}>{busy ? 'Creating…' : 'Create application'}<ArrowRight size={16} aria-hidden="true" /></button></div></form></div><aside className="border-t border-[var(--hairline)] pt-6 lg:border-t-0 lg:border-l lg:pl-8 lg:pt-0"><p className="eyebrow text-stone">Before you start</p><p className="mt-4 text-sm leading-6 text-stone">Have your passport nearby. You will need the bio page, a recent photograph and an email address you can check.</p><p className="mt-6 text-sm leading-6 text-stone">Your application ID will be generated immediately and can be used to return later.</p><p className="mt-6 text-xs leading-5 text-stone">Fictional data only · No real visa application is created.</p></aside></div></main>
}
