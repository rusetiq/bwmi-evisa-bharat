...
export default function Documents() {
  ...
  if (loading) return <PageLoading label="Retrieving your document checklist…" />
  ...
  async function upload(type: string, file: File) {
    ...
    if (!extensionOk || (file.type && !allowed.includes(file.type))) { setFieldErrors((previous) => ({ ...previous, [type]: 'Choose a PDF, JPG or PNG file.' })); return }
    ...
    setUploading(type)
    try { setApplication(await api.uploadDocument(currentApplication.publicId, form)) } catch (requestError) { setFieldErrors((previous) => ({ ...previous, [type]: requestError instanceof ApiError ? requestError.message : "We couldn't upload that file. Try again." })) } finally { setUploading(null) }
  }
