...
export default function Payment() {
  ...
  if (loading) return <PageLoading label="Preparing your demo payment…" variant="payment" />
  ...
  return <main className="shell py-12 sm:py-20"><div className="mb-8 flex flex-wrap items-center justify-between gap-3"><Link className="link text-sm" to={`/application/${currentApplication.publicId}`}>Back to application</Link></div><div className="grid min-w-0 gap-10 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start"><div className="min-w-0"><h1 className="display mt-3 text-[clamp(2.5rem,10vw,3.75rem)] leading-tight">Complete your application payment.</h1><p className="mt-5 max-w-xl text-base leading-7 text-stone">Choose a demo payment method. No card number, bank login or real transaction is requested.</p><div className="mt-8"><Notice title="Demo payment · no real transaction will occur.">This prototype records a fictional payment outcome so you can explore the complete applicant journey.</Notice></div><fieldset className="mt-8"><legend className="label">Payment method</legend><div className="grid gap-3 sm:grid-cols-3">{[['card', 'Card'], ['upi', 'UPI'], ['netbanking', 'Net banking']].map(([value, label]) => <label key={value} className={`flex min-w-0 cursor-pointer items-center gap-3 border p-4 text-sm ${method === value ? 'border-[var(--ink)] bg-[var(--linen)]' : 'border-[var(--hairline)] bg-white'}`}><input type="radio" name="payment-method" value={value} checked={method === value} onChange={() => setMethod(value)} className="shrink-0 accent-[var(--ink)]" /><span className="min-w-0 break-words">{label}</span></label>)}</div></fieldset><details className="mt-6 border-b border-[var(--hairline)] pb-4 text-sm"><summary className="cursor-pointer text-stone">Demo outcome controls</summary><p className="mt-3 text-xs leading-5 text-stone">Visible for demonstration testing only. The default outcome is success.</p><div className="mt-3 flex flex-wrap gap-3">{(['success', 'failed', 'pending'] as const).map((value) => <label key={value} className="inline-flex items-center gap-2 text-sm"><input type="radio" name="outcome" value={value} checked={outcome === value} onChange={() => setOutcome(value)} className="accent-[var(--ink)]" />{value[0].toUpperCase() + value.slice(1)}</label>)}</div></details>{error && <div className="mt-6"><Notice tone="danger">{error}</Notice></div>}<button type="button" className="btn btn-primary mt-8 w-full sm:w-auto" onClick={() => void pay()} disabled={busy}>{busy ? 'Processing…' : 'Simulate payment'}</button></div><PaymentSummary application={currentApplication} /></div></main>
}
