...
export default function PaymentVerify() {
  ...
  return <main className="shell py-12 sm:py-20"><div className="grid gap-12 lg:grid-cols-[minmax(0,640px)_300px] lg:gap-20"><div><Link className="link text-sm" to="/">Back to e-Visa home</Link><div className="mt-10"><SectionHeading eyebrow="Payment verification" title="Check a payment status." description="Use the same application details to view the latest demo payment record." /></div><form className="space-y-6" onSubmit={find} noValidate><TextField id="verify-application-id" label="Application ID" value={applicationId} onChange={setApplicationId} placeholder="IND-EV-26-7K4P9X" required /><div className="grid gap-6 sm:grid-cols-2"><TextField id="verify-passport" label="Passport number" value={passportNumber} onChange={setPassportNumber} required /><TextField id="verify-dob" label="Date of birth" type="date" value={dob} onChange={setDob} required /></div>{error && <Notice tone="danger">{error}</Notice>}<button type="submit" className="btn btn-primary gap-2" disabled={busy}>{busy ? 'Checking…' : 'Check payment'}<Search size={16} aria-hidden="true" /></button></form>{application && <PaymentVerificationResult application={application} onRefresh={() => void refresh(application.publicId, setApplication, setError)} />}</div><aside className="border-t border-[var(--hairline)] pt-6 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0"><ShieldCheck size={20} aria-hidden="true" /><p className="mt-4 text-sm leading-6 text-stone">Your details are used only to locate the fictional record in this prototype. No external payment provider is contacted.</p></aside></div></main>
}

function PaymentVerificationResult({ application, onRefresh }: { application: Application; onRefresh: () => void }) {
  const payment = application.payment
  if (!payment || payment.status === 'UNPAID' || payment.status === 'FAILED') return <section className="mt-12 border-t border-[var(--hairline)] pt-8"><p className="eyebrow text-stone">Payment found</p><div className="mt-3 flex flex-wrap items-center justify-between gap-4"><h2 className="display text-3xl">Payment required.</h2><StatusLabel status={payment?.status ?? 'UNPAID'} /></div><p className="mt-4 text-sm leading-6 text-stone">{payment?.status === 'FAILED' ? 'The last demo attempt did not complete.' : 'This application has not received a successful payment yet.'}</p><Link className="btn btn-primary mt-6" to={`/payment/${application.publicId}`}>Pay now</Link></section>
