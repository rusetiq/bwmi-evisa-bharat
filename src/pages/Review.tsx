...
export default function Review() {
  ...
  const completeSteps = useMemo(() => application ? applicationSteps.every((step) => isStepComplete(step, application.sections?.[step], application.sections)) : false, [application])
  ...
  if (loading) return <PageLoading label="Preparing your review…" variant="application" />
...
function ReviewSection({ title, data, onEdit }: { title: string; data: SectionData; onEdit: () => void }) {
  ...
  return <section className="border-t border-[var(--hairline)] pt-6"><div className="flex min-w-0 flex-wrap items-center justify-between gap-3"><h2 className="display min-w-0 break-words text-3xl">{title}</h2><button type="button" className="link inline-flex shrink-0 items-center gap-1 text-sm" aria-label={`Edit ${title}`} onClick={onEdit}>Edit <Pencil size={13} aria-hidden="true" /></button></div>{rows.length ? <dl className="mt-4">{rows.map((row) => <div key={row.key} className="grid min-w-0 gap-1 border-b border-[var(--hairline)] py-3 sm:grid-cols-[minmax(160px,0.7fr)_minmax(0,1.3fr)]"><dt className="text-xs normal-case tracking-normal text-stone">{humanize(row.key)}</dt><dd className="m-0 min-w-0 break-words text-sm">{row.key.toLowerCase().includes('date') || row.key === 'dob' || row.key === 'proposedArrival' || row.key === 'expectedArrival' || row.key === 'expectedDeparture' ? formatDate(row.value) : row.value}</dd></div>)}</dl> : <p className="mt-4 text-sm text-stone">This section has no saved answers yet.</p>}</section>
}
