...
export default function Submitted() {
  ...
  if (loading) return <PageLoading label="Confirming your submission…" variant="application" />
  ...
  return <main className="shell py-12 sm:py-20"><div className="mx-auto max-w-3xl"><ButtonLink className="link text-sm" to={`/application/${currentApplication.publicId}`}>Open application workspace</ButtonLink><div className="py-12 sm:py-20"><div className="flex h-14 w-14 items-center justify-center border border-[#7a9b82] bg-[#f2f7f3]"><Check className="text-[#356846]" size={26} aria-hidden="true" /></div><h1 className="display mt-3 max-w-2xl text-[clamp(2.5rem,11vw,3.75rem)] leading-tight">Your application has been submitted.</h1><p className="mt-6 max-w-xl text-base leading-7 text-stone">Keep this reference safe. It is the quickest way to return to your application and follow its progress.</p><div className="mt-10 border-y border-[var(--hairline)] py-6"><p className="text-sm font-medium normal-case tracking-normal text-stone">Application ID</p><div className="mt-3 flex flex-wrap items-center gap-3"><code className="break-all text-xl tracking-[.08em]">{currentApplication.publicId}</code><button type="button" className="btn btn-secondary gap-2" onClick={() => void copyId()}><Clipboard size={15} aria-hidden="true" />{copied ? 'Copied' : 'Copy ID'}</button></div></div><div className="mt-8"><Notice title="Fictional demo submission">We have recorded this reference against the mock applicant email <strong>{currentApplication.email || 'provided address'}</strong>. No real email has been sent.</Notice></div><div className="mt-8 flex flex-col gap-3 sm:flex-row"><button type="button" className="btn btn-primary" onClick={() => navigate(`/payment/${currentApplication.publicId}`)}>Continue to payment</button><Link className="btn btn-secondary gap-2" to={`/application/${currentApplication.publicId}/print`}><Printer size={15} aria-hidden="true" />Print application</Link></div></div></div></main>
}
