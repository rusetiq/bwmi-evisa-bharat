...
import { calculateApplicationFee, getApplicationProgress as clientProgress, getRequiredDocuments } from '@/lib/domain'
import { HttpError, now, parseDate } from '../http'
import { applicationFromBundle, createEventStatement, getApplicationBundle, getSectionRows, getVisaTypeRow, requireApplicationRow, upsertSectionStatement } from '../db/queries'
...
export function getApplicationProgress(sections: Record<string, SectionData>) {
  return clientProgress(sections)
}
...
export function normalizeVisaSlug(value: string | undefined | null) {
  const slug = String(value ?? 'e-tourist').trim().toLowerCase().replace(/\s+/g, '-')
  return slug.startsWith('e-tourist-') ? 'e-tourist' : slug
}
...
function deriveApplicationFields(sections: StoredSections, input: { nationality?: string; visaType?: string; visaTypeId?: string; email?: string; proposedArrival?: string; applicantName?: string; dob?: string; passportNumber?: string }) {
  ...
  return {
    ...
    visaType: normalizeVisaSlug(input.visaType ?? input.visaTypeId ?? String(visa.visaSubtype ?? visa.visaType ?? 'e-tourist')),
...
function normalizedStatements(db: Database, applicationId: number, sections: StoredSections, updatedAt: string, affected = Object.keys(sections)) {
  const statements: D1PreparedStatement[] = []
  const applicant = sections.personal ?? {}
  const passport = sections.passport ?? {}
  if (affected.includes('personal')) statements.push(db.prepare(`INSERT INTO applicants (application_id, surname, given_names, gender, birth_city, birth_country, citizenship, religion, identification_marks, education, data_json, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  if (affected.includes('passport')) statements.push(db.prepare(`INSERT INTO passports (application_id, passport_number, passport_type, issuing_country, place_of_issue, issue_date, expiry_date, data_json, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
export async function updateApplication(db: Database, key: string, rawInput: unknown): Promise<{ updatedAt: string; version: number }> {
  ...
  const existingSections = await getSectionRows(db, current.id, current.sections_json)
  ...
  statements.push(...normalizedStatements(db, current.id, sections, updatedAt, Object.keys(incomingSections)))
...
export async function submitApplication(db: Database, key: string): Promise<Application> {
  ...
  const updatedAt = now()
  const amount = calculateApplicationFee({ nationality: current.nationality, visaType: current.visa_type_id, sections: check.bundle.sections }).total
